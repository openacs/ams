...
# first we check to see if there are new options.  if yes we add them
...
# now that all the options are in the database we get the "old" sort
# order if not value for sort_key is provided we will keep the same
# order as before

set option_ids [db_list get_option_ids {select option_id from ams_option_types where attribute_id = :attribute_id order by sort_order}]

# first we get the highest sort_order so variables without a
# sort_order can be given one
...
db_transaction {
    foreach option_id $option_ids {
        ...
        incr highest_sort
        db_dml update_sort_order {update ams_option_types set sort_order = :highest_sort where sort_order = :sort_order and attribute_id = :attribute_id}
